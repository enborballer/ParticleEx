...
void CParticle::ReloadConfig()
{
	...
	InitParticleArray(&CParticleArray::GetPS2()[0], CParticleArray::GetPS2().size());
...
#else
	Int32 slot = CTxdStore::AddTxdSlot("particleREPS2");

	Char Path[MAX_PATH];
	sprintf(Path,"%s\\ParticleEx\\PS2\\PARTICLE.TXD", GetAsiPath().c_str());
...
void CParticle::Shutdown()
{
	...
	slot = CTxdStore::FindTxdSlot("particleREPS2");
	CTxdStore::RemoveTxdSlot(slot);
#endif
...
//#endif
	...
	for ( Int32 i = 0; i < MAX_PARTICLES; i++ )
	{
		...
		for ( ; particle != NULL; _Next(particle, prevParticle, psystem, bRemoveParticle) )
		{
			...
			if (  CTimer::m_snTimeInMilliseconds > particle->m_nTimeWhenWillBeDestroyed || particle->m_nAlpha == 0/*missing on ps2*/ )
			{
...
void CParticle::Render()
{
	...
	for ( Int32 i = 0; i < MAX_PARTICLES; i++ )
	{
		...
		while ( particle != NULL )
		{
			...
			if ( particle->m_nAlpha == 0 ) //missing on ps2
				canDraw = false;
			
			if ( /*missing on ps2*/canDraw && psystem->m_nFinalAnimationFrame != 0 && frames != NULL )
			{
			...
			if ( canDraw && psystem->Flags & DRAWTOP2D )
			{
				if ( particle->m_nRotation != 0 )
				{
					 CSprite::RenderBufferedOneXLUSprite2D_Rotate_Dimension(
							...
							(Float)particle->m_nRotation, //DEG2RAD((Float)particle->m_nRotation) PS2
							particle->m_nAlpha);
				}
				else
				{
