...
//(45)
#define MAX_CARFLAME_FILES        ARRAY_SIZE(CarFlameFiles)
...
RwTexture *gpRainDropTex[MAX_RAINDROP_FILES];
RwRaster  *gpRainDropRaster[MAX_RAINDROP_FILES];

RwTexture *gpCarFlameTex[MAX_CARFLAME_FILES];
RwRaster  *gpCarFlameRaster[MAX_CARFLAME_FILES];
...
void CParticle::InitParticleArray(CParticle *array, UInt32 size)
{
	m_pUnusedListHead = array;
...
void CParticle::ReloadConfig()
{
	...
	InitParticleArray(&CParticleArray::GetXBOX()[0], CParticleArray::GetXBOX().size());
	/*
...
void CParticle::Initialise()
{
...
#else
	Int32 slot = CTxdStore::AddTxdSlot("particleREXBOX");
	
	Char Path[MAX_PATH];
	sprintf(Path,"%s\\ParticleEx\\XBOX\\PARTICLE.TXD", GetAsiPath().c_str());
	...
	for ( Int32 i = 0; i < MAX_CARFLAME_FILES; i++ )
	{
		gpCarFlameTex[i] = RwTextureRead(CarFlameFiles[i], NULL);
		
		if ( gpCarFlameTex[i] != NULL )
			gpCarFlameRaster[i] = RwTextureGetRaster(gpCarFlameTex[i]);
	}
	...
	for ( Int32 i = 0; i < MAX_PARTICLES; i++ )
	{
		...
		switch ( i )
		{
			...
			case PARTICLE_CARFLAME_MOVING:
				if ( gpCarFlameRaster[0] != NULL )
					entry->m_ppRaster = gpCarFlameRaster;
				else
...
void CParticle::Shutdown()
{
	...
	for ( Int32 i = 0; i < MAX_CARFLAME_FILES; i++ )
	{
		if ( gpCarSplashTex[i] )
		{
			RwTextureDestroy(gpCarFlameTex[i]);
			gpCarFlameTex[i] = NULL;
		}
	}
...
#if USE_CUSTOM_DIR == TRUE
	slot = CTxdStore::FindTxdSlot("particleREXBOX");
	CTxdStore::RemoveTxdSlot(slot);
#endif
		...
		for ( ; particle != NULL; _Next(particle, prevParticle, psystem, bRemoveParticle) )
		{
			...
			if ( psystem->m_fGravitationalAcceleration > 0.0f )
			{
				...
				else if ( psystem->Flags & ZCHECK_STEP )
				{
					...
					{
						if ( moveStep.z <= point.m_vecPosition.z )
						{
							moveStep.z = point.m_vecPosition.z;
							...
							case PARTICLE_GUNSHELL:
								{
									...
									PlayOneShotScriptObject(122, particle->m_vecPosition);
								}
								break;
							...
							case PARTICLE_GUNSHELL_BUMP1:
								{
									...
									PlayOneShotScriptObject(123, particle->m_vecPosition);
								}
								break;
			...
			if ( psystem->m_fAnimationSpeed != 0.0f )
			{
	...
	for ( Int32 i = 0; i < MAX_PARTICLES; i++ )
	{
		...
		while ( particle != NULL )
		{
			...
			if ( canDraw )
			{
				...
				{
					if ( (!particleBanned || Float(RsGlobal.w) * fParticleScaleLimit >= w)
											&& Float(RsGlobal.h) * fParticleScaleLimit >= h )
					{
						...
						else if ( psystem->Flags & VERT_TRAIL )
						{
							...
							CSprite::RenderBufferedOneXLUSprite(coors.x, coors.y, coors.z,
									particle->m_fSize * w,
									(particle->m_fSize + fTrailLength * psystem->m_fTrailLengthMultiplier) * h, // bug ?
									particle->m_Color.red,
									particle->m_Color.green,
									particle->m_Color.blue,
									particle->m_nColorIntensity,
...
{
	Log("m_vecPosition(%f, %f, %f)", m_vecPosition.x, m_vecPosition.y, m_vecPosition.z);
	Log("m_vecVelocity(%f, %f, %f)", m_vecVelocity.x, m_vecVelocity.y, m_vecVelocity.z);
	Log("m_vecScreenPosition(%f, %f, %f)", m_vecScreenPosition.x, m_vecScreenPosition.y, m_vecScreenPosition.z);
	Log("m_nTimeWhenWillBeDestroyed(%d)", m_nTimeWhenWillBeDestroyed);
	Log("m_nTimeWhenColorWillBeChanged(%d)", m_nTimeWhenColorWillBeChanged);
	Log("m_fZGround(%f)", m_fZGround);
	Log("m_vecParticleMovementOffset(%f, %f, %f)", m_vecParticleMovementOffset.x, m_vecParticleMovementOffset.y, m_vecParticleMovementOffset.z);
	Log("m_nCurrentZRotation(%d)", m_nCurrentZRotation);
	Log("m_nZRotationTimer(%d)", m_nZRotationTimer);
	Log("m_fCurrentZRadius(%f)", m_fCurrentZRadius);
	Log("m_nZRadiusTimer(%d)", m_nZRadiusTimer);
	Log("m_fSize(%f)", m_fSize);
	Log("m_fExpansionRate(%f)", m_fExpansionRate);
	Log("m_nFadeToBlackTimer(%d)", m_nFadeToBlackTimer);
	Log("m_nFadeAlphaTimer(%d)", m_nFadeAlphaTimer);
	Log("m_nColorIntensity(%d)", m_nColorIntensity);
	Log("m_nAlpha(%d)", m_nAlpha);
	Log("m_nCurrentFrame(%d)", m_nCurrentFrame);
	Log("m_nAnimationSpeedTimer(%d)", m_nAnimationSpeedTimer);
	Log("m_nRotationStep(%d)", m_nRotationStep);
	Log("m_nRotation(%d)", m_nRotation);
	Log("m_Color(%d, %d, %d, %d)", m_Color.red, m_Color.green, m_Color.blue, m_Color.alpha);
	Log("m_pNext(0x%X)", m_pNext);
}
